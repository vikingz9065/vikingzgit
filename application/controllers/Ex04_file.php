...
class Ex04_file extends CI_Controller
{
	...
	*/
	public function make_file()
	{
		...
		// (helper) 파일에 내용 저장하기
		$result = write_file($filepath,"코드이그나이터 파일 저장 예제"); //$result = write_file($filepath,"코드이그나이터 파일 저장 예제", 'r+');

		if($result) {
			$this->output->append_output('<h1>파일 저장함 ::: '.$filepath.'</h1>',"result");			
		} else {
			$this->output->append_output('<h1>파일 저장 실패함</h1>',"result");
		}
